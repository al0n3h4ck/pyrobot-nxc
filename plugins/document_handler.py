...
async def document_handler(client: Client, message: Message):
    print("{} --> Envio un documento!".format(message.from_user.username))
    cola = 0
    if message.from_user.id not in Config.ADMINS:
        return
    else:
        if Config.JOB != 0:
            cola = await client.send_message(chat_id=message.from_user.id, text="Archivo en cola...")
        try:
            Config.JOB += 1
            if cola != 0:
                await cola.delete()
            msg = await client.send_message(chat_id=message.chat.id,
                                            text="Archivo detectado...",
                                            reply_to_message_id=message.message_id
                                            )
            print(message.document.file_name)
            full_name = message.document.file_name
            if full_name is None:
                ext = message.document.mime_type.split("/")[1]
                full_name = message.document.file_unique_id + "." + ext
